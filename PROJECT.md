...
| server-1.3 | **SMTP double-read fix:** `lib/smtp.php` read the "250 OK" after DATA twice → timeout exception *after* successful send → API wrongly reported "mail could not be sent" **and rolled back `email_pending`**, which also broke the verification link. Fixed (single `smtp_expect` after DATA). Note: ALL-INKL SMTP works with port 465 + `'ssl'` (lowercase!) and a real KAS mailbox. |
| server-1.4 | **Admin learning-progress tracking:** `api/admin.php` user detail now returns full stars+done maps; `admin.php` detail view shows per-app progress (coins, total stars, lessons done, last sync) plus a per-lesson star/completion table |
